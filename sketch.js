...
function draw() {
  ...
  if (!src) {
    fill(220);
    textSize(14);
    text("Choose an image. Press S to save outputs.", 12, 60);
    textSize(12);
    text("A and B keep the exact color histogram by rearranging pixels only.", 12, 84);
    return;
  }
  ...
  drawFit(src, pad, pad, cellW, cellH);
  if (outA) drawFit(outA, pad * 2 + cellW, pad, cellW, cellH);
  if (outB) drawFit(outB, pad, pad * 2 + cellH, cellW, cellH);
  if (outCTRL) drawFit(outCTRL, pad * 2 + cellW, pad * 2 + cellH, cellW, cellH);

  fill(235);
  textSize(12);
  text("SRC", pad, pad + 4);
  text("RPC_A histogram perfect", pad * 2 + cellW, pad + 4);
  text("RPC_B more reduced histogram perfect", pad, pad * 2 + cellH + 4);
  text("CTRL tile shuffle of SRC", pad * 2 + cellW, pad * 2 + cellH + 4);
}

function keyPressed() {
  if (key === "s" || key === "S") {
    if (src && outA && outB && outCTRL) saveOutputs();
  }
}
...
function saveOutputs() {
  ...
  saveCanvas(base + "_CANVAS", "png");
  src.save(base + "_SRC", "png");
  outA.save(base + "_RPC_A", "png");
  outB.save(base + "_RPC_B", "png");
  outCTRL.save(base + "_CTRL", "png");
}
